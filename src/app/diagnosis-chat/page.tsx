      ...
      <main className="flex flex-col px-4 pb-4 pt-6">
        <div className="mx-auto flex w-full max-w-3xl flex-col">
          ...
          {/* ★ B안 최종 ★ 카드를 화면 높이만큼 채우되(min-h), 카드 '안'에서
              ...
              세로 길이는 100dvh-680px 로 많이 줄여(대표님 재재재재요청) 카드가 내용물에
              거의 맞게 붙어 카드 안 휑함이 사라지도록 한다. */}
          <div className="flex min-h-[calc(100dvh-680px)] flex-col rounded-2xl border border-gray-100 bg-gray-50/60 p-4 shadow-card">
...
const BotBubble = memo(function BotBubble({ text }: { text: string }) {
  return (
    <div className="flex items-end gap-2">
      <BotAvatar />
      <div className="chat-bot-bubble min-w-[45%] max-w-[88%] whitespace-pre-line break-keep rounded-2xl rounded-bl-md border border-white bg-white px-4 py-3 text-[15px] leading-relaxed text-brand-dark shadow-sm">
        {text}
      </div>
    </div>
  );
