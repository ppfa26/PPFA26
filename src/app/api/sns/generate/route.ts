...
// 대표님 실제 운영 방식(2026 개편): CTA에 링크 2개 고정 노출.
//  · 모바일/PC 겸용 무료 조회(공식 홈페이지): www.mosachin.co.kr
//  · 모바일용 무료 조회(토스 미니앱): minion.toss.im/tsduQsE4
const DEFAULT_LINK_HOME = "https://www.mosachin.co.kr";
const DEFAULT_LINK_TOSS = "https://minion.toss.im/tsduQsE4";
const DEFAULT_LINK_INPOCK = DEFAULT_LINK_HOME; // 하위호환용(더는 인포크 안 씀)

// 토스 쉐어링크(특가템) - 고정. 본문 중간 "오늘의 특가템" 블록에 그대로 사용.
const TOSS_DEAL_LINK = "https://toss.im/_m/v6TXD1n8";

// 출처 푸터(고정) - 글 맨 끝에 그대로.
const SOURCE_FOOTER = "- 출처: 모두의사업친구(모두의사업친구.kr) · 무단 복제·도용 금지";
...
// 특가템 블록(고정) - 본문 마무리 클로징 직전에 그대로 삽입.
const DEAL_BLOCK = `----------------------------------------------
...
※ 본 게시글에는 토스 쉐어링크가
포함되어 있으며, 링크를 통한 구매 시
일정 수수료를 지급받을 수 있습니다.

----------------------------------------------`;

// 마무리 CTA 블록(고정) - 링크 2개(홈페이지 + 토스 미니앱) 노출.
const CTA_BLOCK = `내 사업장이 자격 조건에 해당하는지 아래에서 확인해보세요.
AI가 내 사업장에 알맞은 정부지원사업을 전부 찾아드립니다.

모바일/pc 겸용 무료 조회 (공식 홈페이지)
${DEFAULT_LINK_HOME}
...
━━━━━━━━━━━━━━━━━━━━━━━━
- [금지] 다음 "장식용 특수기호"는 절대 쓰지 말 것: ▶ ▸ ▪ ▶︎ ● ◆ ■ □ ○ · ・ ‧ » › ※ ★ ☆ ✓ ✔ ↳ → 및 마크다운 강조/제목 기호(**, ##). 이런 기호가 들어가면 지저분하고 AI 티가 난다.
- [이모지] 블로그 "본문 설명 문장"에는 이모지를 쓰지 말 것(AI 티가 나서 신뢰 저하). 단, 아래 "고정 블록"의 라벨에는 지정된 이모지를 그대로 쓴다: 특가템 블록의 "📌 대표님들을 위한 오늘의 특가템", 그리고 CTA 안내 문구 끝 "👇" 정도의 가벼운 이모지는 허용. 그 외 본문 문장에 이모지 남발은 금지.
- [허용] 아래 "일반 키보드 문자 기호"는 자연스럽게 사용해도 된다(대표님 지정). 이것들은 장식기호가 아니라 평범한 문장부호라 괜찮다:
   ' , . ! ^^ % # " : / < > [ ] ( ) - _
   · 문장부호(쉼표, 마침표, 느낌표, 따옴표, 콜론 등)는 평소처럼 자연스럽게.
   · 대괄호 [ ] 또는 홑화살괄호 < > : 섹션 제목/소제목을 감쌀 때. 예: [ 지원 대상 및 신청 자격 ]
   · 소괄호 ( ) : 짧은 부연 설명. 예: 지원 한도 (최대 5천만원)
   · 하이픈 - : 구분선 용도. 예: 섹션 사이에 " - - - - - " 한 줄. (문장 머리표로는 쓰지 말 것.)
   · % # / _ 등은 필요할 때만(퍼센트, 해시태그, 날짜/구분 등) 자연스럽게.
   · ^^ 같은 가벼운 표현은 아주 가끔 마무리 인사 정도에만. 남발 금지.
- 허용 기호라도 "남발"하지 말 것. 특히 제목/목차/구분선 외의 본문 문장은 평소처럼 담백한 글로 쓴다.
...
■ 링크 규칙
━━━━━━━━━━━━━━━━━━━━━━━━
- 네이버 블로그 마무리 CTA에는 아래 "링크 2개"를 모두 고정으로 노출한다(순서·문구 그대로, 변경 금지).
  · 모바일/pc 겸용 무료 조회 (공식 홈페이지): ${DEFAULT_LINK_HOME}
  · 모바일용 무료 조회 (토스 미니앱): ${DEFAULT_LINK_TOSS}
- 본문 중간 "오늘의 특가템" 블록의 토스 쉐어링크(${TOSS_DEAL_LINK})와 출처 푸터(${SOURCE_FOOTER})도 고정. 절대 바꾸거나 빼지 말 것.
...
3. 본문 섹션(공고 자료에 있는 항목만, 순서대로. 각 섹션 제목은 [ ] 로 감싼 한 줄 제목으로. 예: [ 지원 대상 및 신청 자격 ]):
   ...
   - 신청 시 필요 서류 (자료에 있으면)
   - 우대 지원 혜택 (자료에 있으면)
4. 특가템 블록(고정, 반드시 이 위치): "우대 지원 혜택"까지 본문 섹션이 끝난 직후, 감성 마무리 문단보다 "먼저" 아래 블록을 "글자 그대로" 넣는다(수정/생략 금지). 앞뒤로 빈 줄 하나씩 둔다.
${DEAL_BLOCK}
5. 감성 마무리: 이 정부지원사업의 의미를 짧게 정리하는 클로징 한 문단(특가템 블록 다음).
6. 핵심 정리: "[ 핵심 정리 ]"라는 제목(대괄호로 감쌈) 아래, 정부지원사업명/목적/주관/사업수행기관/지원대상/지원내용/지원한도/신청기간/신청방법/문의처 등 핵심을 "항목명: 내용" 형식으로 줄줄이 요약(자료에 있는 항목만, 금지 기호는 쓰지 말 것).
7. 마무리 CTA(고정, 반드시 이 형태로. 링크 2개 모두 그대로. 변형/축약/링크 변경 금지):
${CTA_BLOCK}
   ("행정대행 아님, 수수료 0원, 대출 상담 아님" 같은 부정형/방어성 문구는 절대 넣지 말 것.)
8. 출처 푸터(고정, 태그보다 뒤 맨 마지막 줄): 아래 한 줄을 글자 그대로 맨 끝에 넣는다.
${SOURCE_FOOTER}
9. 사진 자리(매우 중요, 반드시 지킬 것): 본문에 사진 자리를 "정확히 3개" 넣는다. 4개나 8개가 아니라 반드시 3개. 표기는 오직 [사진1] [사진2] [사진3] 형태로 "번호만" 쓴다. 절대로 설명이나 콜론(:)이나 추천 문구를 붙이지 말 것. 예: "[사진1: 표지 이미지]" (X, 금지), "[사진1]" (O, 이렇게만). 위치는 다음과 같이 고정한다.
   - [사진1]: "이 정부지원사업의 특징" 섹션 다음
   - [사진2]: "지원 조건 및 한도"(또는 신청 절차) 섹션 다음
   - [사진3]: "신청 시 필요 서류" 섹션 앞
   각 사진 자리는 한 줄에 [사진N] 딱 그것만. 대괄호 표기는 허용(특수기호 아님). 3개보다 적거나 많으면 안 된다.
10. 톤과 분량(대표님 지정, 중요): 전문적이고 정중한 정보 전달체. 딱딱한 관공서 말투는 피하고, 전문가가 사장님 눈높이로 쉽게 풀어주는 느낌.
   ...
   - 전체 구조(오프닝, 질문후킹, 본문 섹션, 특가템 블록, 감성 마무리, 핵심 정리, CTA, 출처 푸터, 사진 3개)는 그대로 유지하되, 각 문단의 길이만 군살 없이 줄인다.
11. 줄바꿈(매우 중요, 대표님 지정 스타일): 복사해서 바로 붙여넣을 수 있도록 "짧게 끊어" 쓴다. 한 줄이 길어서 모바일에서 가로로 넘치면 안 된다.
   ...
   (O) 선정 기업은 현지 바이어 발굴과\\n1대1 수출상담 주선,\\n쇼케이스 형태의 상담장 운영 등\\n미국 시장 진출을 위한\\n현장형 지원을 받게 됩니다.
12. 섹션 간격(매우 중요, 대표님 지정): 답답하게 붙이지 말고 아래 간격을 반드시 지킨다.
...
━━━━━━━━━━━━━━━━━━━━━━━━
{
  "channels": [
    { "channel": "네이버 블로그", "emoji": "📝", "depth": "심층", "blocks": [
      ...
      { "key": "blog-body", "label": "본문 (오프닝3줄+질문후킹+섹션+특가템+마무리+핵심정리+CTA+출처, [사진1]~[사진3] 3개 포함)", "hint": "우리 블로그 표준 구조. 섹션 제목은 [ ] 로 감싸 가독성 있게(금지 기호는 금지, 이모지는 특가템/CTA 고정 블록에만). [사진1]~[사진3] 총 3개 반드시 포함. 특가템 블록·CTA 링크 2개·출처 푸터는 고정 문구 그대로.", "text": "안녕하세요 대표님!\\n정부지원사업 AI 통합 매칭 플랫폼\\n모두의사업친구입니다.\\n\\n..." },
      { "key": "blog-tags", "label": "태그 12~15개", "text": "#... #모두의사업친구 #모사친" }
    ] }
  ]
}
JSON 외 다른 텍스트는 절대 출력하지 마세요.
모든 text 필드는 실제 줄바꿈이 반영되도록 \\n을 정확히 사용하세요. 위에서 [금지]한 장식기호(▶ ▸ ● ■ · ※ ★ 등)와 이모지는 어떤 필드에도 절대 넣지 마세요. 허용된 일반 문자 기호( ' , . ! ^^ % # " : / < > [ ] ( ) - _ )는 자연스럽게 쓰되 남발하지 마세요.
...
${DEFAULT_LINK_HOME}
${DEFAULT_LINK_TOSS}
본문 중간 특가템 쉐어링크: ${TOSS_DEAL_LINK}
맨 끝 출처 푸터: ${SOURCE_FOOTER}`;
