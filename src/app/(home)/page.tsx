...
export default function Home() {
  ...
  //  · 너무 빨리 늘면 안 믿기므로(대표님 요청) 하루당 '평균 +10'만 꾸준히 증가.
  //    딱 +10 으로 떨어지면 조작 티가 나므로, 날짜 시드 의사난수로 하루당 +8~+12 로
  //    미세하게 흔들며 누적 → 하루 10건 정도씩 자연스럽게 상승.
  ...
  useEffect(() => {
    ...
    // 지난 날짜만큼 하루치 증가폭(8~12, 평균 10)을 누적 - 꾸준히 상승(대표님 요청)
    let users = BASE_USERS;
    for (let i = 1; i <= daysPassed; i++) {
      users += 8 + Math.floor(seeded(i) * 5); // 8 ~ 12 (평균 10)
    }
  ...
  return (
    <PageShell pageKey="home">
      ...
      <main className="pb-24 sm:pb-0">
            ...
            {BETA_FREE && (
              <div className="mx-auto mt-5 flex w-fit items-center gap-2 rounded-full bg-brand-orange/10 px-4 py-1.5 sm:mt-5">
                ...
                <span className="break-keep text-[12.5px] font-bold text-brand-dark sm:text-sm">
                  ...
                  <span className="font-black text-brand-orange">전액 무료</span>
                </span>
              </div>
            )}
                ...
                <div className="space-y-3 p-4 text-left sm:p-6">
                  ...
                  >
                    <p className="break-keep text-[15px] font-extrabold leading-snug text-brand-dark sm:text-[16px]">
                      <span className="text-brand-orange">🔒 잠긴 부분</span>은 진단 완료 시 전부 공개됩니다.
                    </p>
        ...
        {/* 구간 구분 - 얇은 회색 가로줄 */}
        <div className="section-divider" aria-hidden="true" />
        {/* 섹션 상하 여백 - 하단부(FAQ→CTA→광고→푸터) 전체를 한 리듬으로 균형있게 통일
            (대표님 요청: 세로 공백이 들쭉날쭉 → py-7 sm:py-9 로 촘촘·일관되게) */}
        <section className="bg-gray-50 px-4 py-7 sm:py-9">
          <div className="reveal mx-auto max-w-3xl">
