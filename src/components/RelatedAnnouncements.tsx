...
export default function RelatedAnnouncements({
  ...
  // 로딩 중 또는 표시할 공고 없음 → 섹션 자체를 숨김(빈 카드 방지)
  if (loading) {
    return (
      <section className="mt-5">
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-card sm:p-6">
          <p className="text-sm text-brand-dark/50">📢 관련 공고를 불러오는 중…</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="mt-5">
      <div className="rounded-3xl border border-brand-primary/20 bg-white p-5 shadow-card sm:p-6">
        <div className="mb-1 flex items-center gap-2">
          <span className="text-lg">📢</span>
          <h2 className="break-keep text-lg font-black text-brand-dark sm:text-xl">
            지금 열려있는 관련 정부지원사업
          </h2>
        </div>
        <p className="mb-4 break-keep text-xs text-brand-dark/60 sm:text-sm">
          진단 정보(지역·업종·관심분야)를 바탕으로 현재 공고 중인 실제 사업을 추렸습니다.
        </p>
