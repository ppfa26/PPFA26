...
${region}에서 사업을 운영하고 계신가요?
사업에 필요한 자금이 부족하신가요?
좋은 조건으로 자금을 마련하고 싶으신가요?
${josa(i.title, "이/가")} 있다는 것을 아시나요?

이 제도는 ${region} 소상공인의 사업 안정과 성장을 지원하는 매우 실용적인 정책입니다. 오늘은 신청 자격부터 조건, 절차, 주의사항까지 하나씩 정리해드리겠습니다.
...
${josa(i.title, "은/는")} ${josa(i.target, "이/가")} 사업에 필요한 자금을 좋은 조건으로 마련할 수 있도록 지원하는 제도입니다. 규모가 큰 기업이 아니라, 오히려 규모가 크지 않은 소상공인을 위해 마련된 제도라는 점이 핵심입니다.
...
원래 39,900원인 진단을 지금은 오픈 베타로 무료 제공하고 있습니다. 행정대행이 아니라 맞는 공고를 찾아드리는 매칭 서비스이며, 수수료는 0원입니다.
...
내 사업장이 자격 조건에 해당되는지 아래 링크를 통해서 확인해 보세요.

모두의사업친구 정부지원사업 AI 통합 매칭 플랫폼
AI가 내 사업장에 알맞는 정부지원사업을 전부 찾아드립니다.
${i.linkHome}`;
...
내 업종도 해당되는지 궁금하시면 사업자번호만 넣으면 30초 만에 확인됩니다. 지금은 오픈 베타로 무료이고, 수수료도 없습니다. 필요하신 사장님들 참고하세요.
...
export function buildAllChannels(i: SnsInput): SnsChannel[] {
  return [
    ...
    { channel: "당근 비즈프로필", emoji: "🥕", depth: "간단", blocks: buildDaangn(i) },
  ];
}
...
export const UPLOAD_GUIDES: { channel: string; emoji: string; steps: string[] }[] = [
  ...
  },
  {
    ...
    emoji: "📸",
    steps: [
      "카드 이미지 8장을 먼저 준비 (썸네일 스타일)",
      "인스타 앱 > [+] > '게시물' > '여러 항목 선택'으로 카드 1~8번 순서대로",
      "'인스타그램 - 캡션' 복사해서 붙여넣고, 그 아래 '해시태그' 붙여넣기",
      "'공유하기' (링크는 프로필 하단에 한 번만 설정)",
    ],
  },
  {
    channel: "스레드 (이어달기 5개)",
    ...
    steps: [
      "스레드 앱 > [+] '새 스레드'",
      "'스레드 - 이어달기'에서 [본문] 부분만 복사해 첫 칸에 붙여넣기",
      "'스레드에 추가'로 이어달기 2~5번째를 순서대로 복사해 붙여넣기",
      "맨 아래 '해시태그' 붙여넣고 '게시' (첫 30분 댓글 답변이 핵심)",
    ],
  },
  {
    channel: "카카오톡 채널",
    emoji: "💬",
    steps: [
      "카카오 채널 관리자센터(pf.kakao.com) > 내 채널 > '포스트'",
      "'소식 제목' 복사해서 제목에 붙여넣기",
      "'카카오톡 채널 글' 복사해서 본문에 붙여넣고 썸네일 1장 첨부",
      "발행 (낮~저녁 발송 추천)",
    ],
  },
  {
    ...
    emoji: "🥕",
    steps: [
      "당근 비즈니스 앱 > 비즈프로필 > '소식/단골쿠폰' 글쓰기",
      "'제목' 복사해서 붙여넣기",
      "'당근 글' 복사해서 본문에 붙여넣고 썸네일 1장 첨부",
      "동네 범위 확인 후 게시",
    ],
  },
];
