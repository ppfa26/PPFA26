...
export const STEP3_CONDITIONAL_FIELDS = {
  ...
  revenueGrowth2y: {
    label: "최근 2년간 매출 또는 고용이 눈에 띄게 성장하셨나요?",
    hint: "예: 2년 전보다 매출이 크게 늘었거나 직원을 새로 채용해 늘린 경우 (소진공 '성장가속화자금' 매칭용)",
  ...
  govSelected: {
    ...
    hint: "소진공·중기부 등에서 우수 소상공인으로 선정·지정받은 경우 (소진공 '강한소상공인자금' 매칭용)",
    opts: ["예, 선정됐어요", "아니요·해당 없음"],
  },
  // 혁신형 졸업후보 — 정책자금 졸업후보 / 직접대출 성실상환 이력
  policyFundGood: {
    label: "기존 정책자금을 성실히 상환하고 계시거나, 졸업(상환완료)을 앞두고 계신가요?",
    hint: "소진공·중진공 등 정책자금 직접대출을 연체 없이 상환 중이거나 곧 상환완료 예정인 경우 (소진공 '졸업후보기업자금' 매칭용)",
  ...
  reFounder: {
    ...
    hint: "폐업 후 재창업하신 재도전 소상공인 (소진공 '재도전특별자금' 매칭용)",
    opts: ["예, 재도전 중이에요", "아니요·해당 없음"],
  },
  // 대환대출 — 고금리 대출을 저금리 정책자금으로 전환 희망
  wantsRefinance: {
    label: "현재 이용 중인 고금리 대출을 저금리 정책자금으로 갈아타기(대환)를 원하시나요?",
    hint: "카드론·2금융권 등 고금리 대출을 소진공 저금리 자금으로 전환 (소진공 '대환대출자금' 매칭용)",
    opts: ["예, 원해요", "아니요·해당 없음"],
  },
  // 민간투자매칭 — 민간 투자유치 이력/진행
  privateInvestment: {
    label: "엔젤·벤처캐피탈(VC) 등 민간 투자를 유치했거나 진행 중이신가요?",
    hint: "민간 투자유치 실적이 있는 경우 정부가 매칭 지원 (소진공 '민간투자연계매칭융자' 매칭용)",
