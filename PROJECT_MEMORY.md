...
- **(이번 세션) 상단(헤더)·하단(푸터) 여백 안쪽 정리 + 전체 살짝 확대 (A안)** [대표님 요청]
  ...
  - **해법(A안, 순수 표시 조정 — 스코어링/데이터 무관)**: ① 헤더 `pl-2 pr-2 xs:pl-4 xs:pr-3.5 sm:px-6` → `pl-4 pr-4 xs:pl-6 xs:pr-5 sm:px-8`(안쪽으로 당김). ② 푸터 `px-4` → `px-6 sm:px-8`(안쪽으로 당김). ③ PC뷰 기준폭 `DESKTOP_WIDTH` 980 → **900**(전체 콘텐츠 약 9% 확대, 레이아웃 안 깨짐).
